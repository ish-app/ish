...
void tlb_refresh(struct tlb *tlb, struct mmu *mmu) {
    if (ISH_LIKELY(tlb->mmu == mmu && tlb->mem_changes == mmu->changes))
        return;
    tlb->mmu = mmu;
...
__no_instrument void *tlb_handle_miss(struct tlb *tlb, addr_t addr, int type) {
    // Check for memory changes first to avoid unnecessary work
    if (ISH_UNLIKELY(tlb->mmu->changes != tlb->mem_changes)) {
        tlb_flush(tlb);
    }
