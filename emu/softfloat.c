...
static inline float32 packFloat32( bool zSign, int16_t zExp, uint32_t zSig )
{
...
static float32 roundAndPackFloat32( bool zSign, int16_t zExp, uint32_t zSig )
{
    int8_t roundingMode;
    bool roundNearestEven;
    int8_t roundIncrement, roundBits;
...
static float32
 normalizeRoundAndPackFloat32( bool zSign, int16_t zExp, uint32_t zSig )
{
    int8_t shiftCount;
...
static inline float64 packFloat64( bool zSign, int16_t zExp, uint64_t zSig )
{
...
static float64 roundAndPackFloat64( bool zSign, int16_t zExp, uint64_t zSig )
{
    int8_t roundingMode;
    bool roundNearestEven;
...
static float64
 normalizeRoundAndPackFloat64( bool zSign, int16_t zExp, uint64_t zSig )
{
    int8_t shiftCount;
...
static inline floatx80 packFloatx80( bool zSign, int32_t zExp, uint64_t zSig )
{
    ...
    z.signif = zSig;
    z.signExp = ( ( (uint16_t) zSign )<<15 ) + zExp;
    return z;
...
static floatx80
 roundAndPackFloatx80(
     int8_t roundingPrecision, bool zSign, int32_t zExp, uint64_t zSig0, uint64_t zSig1
 )
{
    int8_t roundingMode;
...
static inline uint64_t extractFloat128Frac0( float128 a )
{
    ...
    return a.signExp & LIT64( 0x0000FFFFFFFFFFFF );
 ...
 packFloat128( bool zSign, int32_t zExp, uint64_t zSig0, uint64_t zSig1 )
{
    ...
    z.signif = zSig1;
    z.signExp = ( ( (uint64_t) zSign )<<63 ) + ( ( (uint64_t) zExp )<<48 ) + zSig0;
    return z;
...
static float128
 roundAndPackFloat128(
     bool zSign, int32_t zExp, uint64_t zSig0, uint64_t zSig1, uint64_t zSig2 )
{
    int8_t roundingMode;
    bool roundNearestEven, increment, isTiny;
...
static float128
 normalizeRoundAndPackFloat128(
     bool zSign, int32_t zExp, uint64_t zSig0, uint64_t zSig1 )
{
    int8_t shiftCount;
    uint64_t zSig2;
...
int64_t float32_to_int64_round_to_zero( float32 a )
{
    bool aSign;
    int16_t aExp, shiftCount;
    uint32_t aSig;
...
static float32 addFloat32Sigs( float32 a, float32 b, bool zSign )
{
...
static float32 subFloat32Sigs( float32 a, float32 b, bool zSign )
{
    int16_t aExp, bExp, zExp;
    uint32_t aSig, bSig, zSig;
    int16_t expDiff;
...
static float64 addFloat64Sigs( float64 a, float64 b, bool zSign )
{
    int16_t aExp, bExp, zExp;
...
static float64 subFloat64Sigs( float64 a, float64 b, bool zSign )
{
    int16_t aExp, bExp, zExp;
    uint64_t aSig, bSig, zSig;
    int16_t expDiff;
...
int64_t floatx80_to_int64( floatx80 a )
{
    bool aSign;
    int32_t aExp, shiftCount;
    uint64_t aSig, aSigExtra;
...
int64_t floatx80_to_int64_round_to_zero( floatx80 a )
{
    bool aSign;
    int32_t aExp, shiftCount;
    uint64_t aSig;
...
floatx80 floatx80_round_to_int( floatx80 a )
{
    ...
    roundingMode = float_rounding_mode;
    if ( roundingMode == float_round_nearest_even ) {
        z.signif += lastBitMask>>1;
        if ( ( z.signif & roundBitsMask ) == 0 ) z.signif &= ~ lastBitMask;
    }
    else if ( roundingMode != float_round_to_zero ) {
        if ( extractFloatx80Sign( z ) ^ ( roundingMode == float_round_up ) ) {
            z.signif += roundBitsMask;
        }
    }
    z.signif &= ~ roundBitsMask;
    if ( z.signif == 0 ) {
        ++z.signExp;
        z.signif = LIT64( 0x8000000000000000 );
    }
    if ( z.signif != a.signif ) float_exception_flags |= float_flag_inexact;
    return z;
...
static floatx80 subFloatx80Sigs( floatx80 a, floatx80 b, bool zSign )
{
    ...
    if ( aExp == 0x7FFF ) {
        ...
        float_raise( float_flag_invalid );
        z.signif = floatx80_default_nan_low;
        z.signExp = floatx80_default_nan_high;
        return z;
    }
    if ( aExp == 0 ) {
        aExp = 1;
...
bool floatx80_eq( floatx80 a, floatx80 b )
{
    ...
    return
        ...
        && (    ( a.signExp == b.signExp )
             || (    ( a.signif == 0 )
                  && ( (uint16_t) ( ( a.signExp | b.signExp )<<1 ) == 0 ) )
           );
...
bool floatx80_le( floatx80 a, floatx80 b )
{
    ...
    if ( aSign != bSign ) {
        return
               aSign
            || (    ( ( (uint16_t) ( ( a.signExp | b.signExp )<<1 ) ) | a.signif | b.signif )
                 == 0 );
    }
    return
          aSign ? le128( b.signExp, b.signif, a.signExp, a.signif )
        : le128( a.signExp, a.signif, b.signExp, b.signif );
...
bool floatx80_lt( floatx80 a, floatx80 b )
{
    ...
    return
          aSign ? lt128( b.signExp, b.signif, a.signExp, a.signif )
        : lt128( a.signExp, a.signif, b.signExp, b.signif );
...
int64_t float128_to_int64_round_to_zero( float128 a )
{
    bool aSign;
    int32_t aExp, shiftCount;
    uint64_t aSig0, aSig1;
    ...
    if ( 0 < shiftCount ) {
        if ( 0x403E <= aExp ) {
            ...
            if (    ( a.signExp == LIT64( 0xC03E000000000000 ) )
...
float128 float128_round_to_int( float128 a )
{
    ...
    if ( 0x402F <= aExp ) {
        ...
        if ( roundingMode == float_round_nearest_even ) {
            if ( lastBitMask ) {
                add128( z.signExp, z.signif, 0, lastBitMask>>1, &z.signExp, &z.signif );
                if ( ( z.signif & roundBitsMask ) == 0 ) z.signif &= ~ lastBitMask;
            }
            else {
                if ( (int64_t) z.signif < 0 ) {
                    ++z.signExp;
                    if ( (uint64_t) ( z.signif<<1 ) == 0 ) z.signExp &= ~1;
                }
            }
        }
        else if ( roundingMode != float_round_to_zero ) {
            if (   extractFloat128Sign( z )
                ...
                add128( z.signExp, z.signif, 0, roundBitsMask, &z.signExp, &z.signif );
            }
        }
        z.signif &= ~ roundBitsMask;
    }
        ...
        if ( aExp < 0x3FFF ) {
            if ( ( ( (uint64_t) ( a.signExp<<1 ) ) | a.signif ) == 0 ) return a;
            float_exception_flags |= float_flag_inexact;
            aSign = extractFloat128Sign( a );
            switch ( float_rounding_mode ) {
             case float_round_nearest_even:
        ...
        roundingMode = float_rounding_mode;
        if ( roundingMode == float_round_nearest_even ) {
            z.signExp += lastBitMask>>1;
            if ( ( ( z.signExp & roundBitsMask ) | a.signif ) == 0 ) {
                z.signExp &= ~ lastBitMask;
            }
        }
        else if ( roundingMode != float_round_to_zero ) {
            if (   extractFloat128Sign( z )
                 ^ ( roundingMode == float_round_up ) ) {
                z.signExp |= ( a.signif != 0 );
                z.signExp += roundBitsMask;
            }
        }
        z.signExp &= ~ roundBitsMask;
    }
    if ( ( z.signif != a.signif ) || ( z.signExp != a.signExp ) ) {
        float_exception_flags |= float_flag_inexact;
    }
    return z;
...
static float128 subFloat128Sigs( float128 a, float128 b, bool zSign )
{
    ...
    if ( aExp == 0x7FFF ) {
        ...
        float_raise( float_flag_invalid );
        z.signif = float128_default_nan_low;
        z.signExp = float128_default_nan_high;
        return z;
    }
    if ( aExp == 0 ) {
        aExp = 1;
...
bool float128_eq( float128 a, float128 b )
{
    ...
    return
        ...
        && (    ( a.signExp == b.signExp )
             || (    ( a.signif == 0 )
                  && ( (uint64_t) ( ( a.signExp | b.signExp )<<1 ) == 0 ) )
           );
...
bool float128_le( float128 a, float128 b )
{
    ...
    if ( aSign != bSign ) {
        return
               aSign
            || (    ( ( (uint64_t) ( ( a.signExp | b.signExp )<<1 ) ) | a.signif | b.signif )
                 == 0 );
    }
    return
          aSign ? le128( b.signExp, b.signif, a.signExp, a.signif )
