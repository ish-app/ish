...
static int tty_get(int type, int num, struct tty **tty_out) {
    ...
    if (tty == NULL) {
        ...
        pthread_cond_init(&tty->buf_ready_changed, NULL);
...
int tty_input(struct tty *tty, const char *input, size_t size) {
    ...
    // TODO all kinds of insanely complicated things
    if (!(lflags & ICANON_)) {
        TODO("noncanonical mode");
    }
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (ch == cc[VERASE_] || ch == cc[VKILL_]) {
            ...
            tty->buf_ready = true;
            signal(tty, buf_ready_changed);
            while (tty->buf_ready)
                wait_for(tty, buf_ready_changed);
        } else {
            if (tty->bufsize < 4096)
                tty->buf[tty->bufsize++] = ch;
        }
...
static ssize_t tty_read(struct fd *fd, void *buf, size_t bufsize) {
    ...
    tty->bufsize -= bufsize;
    memmove(buf, buf + tty->bufsize, tty->bufsize); // magic!
    tty->buf_ready = false;
    signal(tty, buf_ready_changed);
